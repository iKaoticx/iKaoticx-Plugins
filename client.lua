...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity == true then
		...
		TriggerEvent("chatMessage", "^1[IDENTITY]", {255, 255, 0}, "Você deve registrar um personagem para jogar.")
	end
end)
	...
	for theData, value in pairs(myIdentity) do
		...
		elseif theData == "dateofbirth" then
			if value == "invalid" then
				reason = "Data de nascimento ~r~inválida~s~!"
				break
			end
...
function verifyName(name)
	...
	if nameLength > 15 or nameLength < 4 then
		return 'Seu nome é muito ~r~longo~s~ ou muito ~r~curto~s~.'
	end
	
	-- Don't allow special characters (doesn't always work)
	local count = 0
	for i in name:gmatch('[aaãbcdefghijklmnoóôõpqrstuvwxyzåäöABCDEFGHIJKLMNOPQRSTUVWXYZÅÄÖ0123456789 -]') do
		count = count + 1
	end
	...
	if spacesInName > 2 then
		return 'Seu nome contém ~y~dois~s~ espaços'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Seu nome deve ~r~começar com letra maiúscula~s~.'
	end
